...
public class RestoreInvConfigScreenHandler extends GenericContainerScreenHandler {
    ...
    public void onSlotClick(int slotIndex, int button, SlotActionType actionType, PlayerEntity player) {
        if (slotIndex < 0 || slotIndex >= this.getInventory().size()) return;
        ...
        switch (slotIndex) {
            case 1:
                if (isLeft) { storage.autoSaveInterval1 = Math.min(60, storage.autoSaveInterval1 + 1); redraw(sp, "Slot 1 Intervall: " + storage.autoSaveInterval1 + " min"); }
                return;
            case 2:
                if (isLeft) { storage.autoSaveInterval1 = Math.max(1, storage.autoSaveInterval1 - 1); redraw(sp, "Slot 1 Intervall: " + storage.autoSaveInterval1 + " min"); }
                return;
            case 10:
                if (isLeft) { storage.autoSaveInterval2 = Math.min(60, storage.autoSaveInterval2 + 1); redraw(sp, "Slot 2 Intervall: " + storage.autoSaveInterval2 + " min"); }
                return;
            case 11:
                if (isLeft) { storage.autoSaveInterval2 = Math.max(1, storage.autoSaveInterval2 - 1); redraw(sp, "Slot 2 Intervall: " + storage.autoSaveInterval2 + " min"); }
                return;
            case 3:
                if (isLeft) { storage.showSaveMessages = !storage.showSaveMessages; redraw(sp, "Chat-Benachrichtigungen " + (storage.showSaveMessages ? "AN" : "AUS")); }
                return;
            case 4:
                if (isLeft) {
            ...
            case 5:
                ...
                return;
            case 7:
                if (isLeft) { storage.playRestoreSound = !storage.playRestoreSound; redraw(sp, "Restore-Sound " + (storage.playRestoreSound ? "AN" : "AUS")); }
                return;
            case 8:
                if (isLeft) { storage.autoSaveOnDeath = !storage.autoSaveOnDeath; redraw(sp, "Auto-Save bei Tod " + (storage.autoSaveOnDeath ? "AN" : "AUS")); }
                return;
            case 13:
                if (isLeft) {
                ...
                return;
            case 14:
                if (isLeft) { storage.requireOpForRestore = !storage.requireOpForRestore; redraw(sp, "Restore nur fuer OPs: " + (storage.requireOpForRestore ? "AN" : "AUS")); }
                return;
            case 18:
                if (isLeft) {
                    storage.saveConfig();
