...
public class RestoreInvMod implements ModInitializer {
    ...
    public void onInitialize() {
        ...
        ServerLifecycleEvents.SERVER_STOPPING.register(server -> storage.saveConfig());
        ...
        ServerLivingEntityEvents.ALLOW_DEATH.register((entity, damageSource, damageAmount) -> {
            if (entity instanceof ServerPlayerEntity sp && storage.autoSaveOnDeath) {
                try { storage.saveInventory(sp, 2); } catch (Throwable t) { t.printStackTrace(); }
            }
            return true;
        });

        ServerTickEvents.END_SERVER_TICK.register(server -> {
            if (storage.getOnlinePlayers(server).isEmpty()) return;
            ticksUntilSlot1--;
            ticksUntilSlot2--;
            if (ticksUntilSlot1 <= 0) {
                for (ServerPlayerEntity p : storage.getOnlinePlayers(server)) storage.saveInventory(p, 0);
                ticksUntilSlot1 = 20L * 60 * Math.max(1, storage.autoSaveInterval1);
            }
            if (ticksUntilSlot2 <= 0) {
                for (ServerPlayerEntity p : storage.getOnlinePlayers(server)) storage.saveInventory(p, 1);
                ticksUntilSlot2 = 20L * 60 * Math.max(1, storage.autoSaveInterval2);
            }
        });
