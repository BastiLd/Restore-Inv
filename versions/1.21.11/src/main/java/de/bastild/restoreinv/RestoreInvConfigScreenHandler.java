            ...
            if (actionType == SlotActionType.PICKUP) { // Left click
                if (clickedStack.getItem() == Items.EMERALD) { // Increase button
                    if (slotIndex == 1) { // Slot 1 increase
                        ...
                        storage.updateConfigGUI(this);
                        player.sendMessage(
                                Text.literal("Slot 1 Intervall auf " + storage.autoSaveInterval1 + " Minuten erh̦ht!"),
                                false);
                        handled = true;
                        ...
                        storage.updateConfigGUI(this);
                        player.sendMessage(
                                Text.literal("Slot 2 Intervall auf " + storage.autoSaveInterval2 + " Minuten erh̦ht!"),
                                false);
                        handled = true;
                    }
